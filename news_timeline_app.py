...
def group_articles_by_topic(articles, similarity_threshold=0.3, query="", day_summaries=None):
    ...
    if days_needing_summaries and (MODEL_PROVIDER == "openai" and OPENAI_AVAILABLE):
        print(f"Generating {len(days_needing_summaries)} new daily summaries: {', '.join(days_needing_summaries)}")
